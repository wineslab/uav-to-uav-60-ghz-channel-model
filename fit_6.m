...
X = x.';
B_ci = X \ y';
fit_line_ci_6 = fpl_reference_1m + B_ci * 10 * log10(distance);
...
sigma_sq_db_ci_6 = sum(diff.^2)/length(diff);
...
X = [ones(1, length(x)); x].';
B_abg = X \ y';
fit_line_abg_6 = B_abg(1) + B_abg(2) * 10 * log10(distance);
...
sigma_sq_db_abg_6 = sum(diff.^2)/length(diff);
...
figure, hold on,
plot(distance, fit_line_ci_6, strcat('-'), 'DisplayName', strcat('CI, h = ', num2str(altitude)))
plot(distance, fit_line_abg_6, strcat('-.'), 'DisplayName', strcat('ABG, h = ', num2str(altitude)))
scatter(distance, min_pathloss_6, markers{1}, 'DisplayName', strcat('h = ', num2str(altitude)))
...
distance_6 = distance;
save('dataset_best_fit_6.mat', 'distance_6', 'fit_line_abg_6', 'fit_line_ci_6')